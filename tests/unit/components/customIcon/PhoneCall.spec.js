import {shallowMount} from "@vue/test-utils";
import PhoneCall from "@/components/customIcon/PhoneCall";

describe("PhoneCall Component", () => {
    let wrapper;
    beforeAll(() => {
        wrapper = shallowMount(PhoneCall);
    });
    test("Check if component is a vue instance", () => {
        expect(wrapper.isVueInstance).toBeTruthy();
    });

    test("It should have a created hook of undefined because component is yet to be defined", () => {
        expect(typeof PhoneCall.created).toBe("undefined");
    });

    test("It should have a name of PhoneCall", () => {
        expect( wrapper.name()).toBe("PhoneCall");
    });

    test("It should not have a name of PhoneCall", () => {
        expect( wrapper.name()).not.toBe("some_gibberish");
    });

    test("It should be an svg tag ", () => {
        const svgElementExist = wrapper.contains('svg')
        expect(svgElementExist).toBe(true)
    });

});